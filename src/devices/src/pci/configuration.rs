...
//
// Ported into libkrun from Cloud Hypervisor's `pci` crate for the PCIe
// passthrough work (CUDA_PASSTHROUGH_IMPL.md). Trimmed of the serde snapshot
// state, the vm-migration hooks and the live MsixConfig back-reference (the
// MSI-X config plumbing lands with step 9); byteorder replaced with native
// little-endian helpers so the module carries no new crate dependency.

use std::result;

use thiserror::Error;

use super::device::BarReprogrammingParams;
...
/// A PCI subclass. Each class in `PciClassCode` can specify a unique set of
/// subclasses. This trait is implemented by each subclass. It allows use of a
/// trait object to generate configurations.
...
/// A PCI capability list. Devices can optionally specify capabilities in their
/// configuration space.
...
impl PciConfiguration {
    ...
    pub fn set_irq(&mut self, line: u8, pin: u32) {
    ...
    pub fn write_config_register(
        ...
        match data.len() {
            ...
            4 => self.write_reg(reg_idx, read_le_u32(data)),
            _ => (),
        }
    ...
    /// The register index of the MSI-X capability, if one was added.
    pub fn msix_cap_reg_idx(&self) -> Option<usize> {
